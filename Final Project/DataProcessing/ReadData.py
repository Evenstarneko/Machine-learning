        ...
        if k % 25 == 0:
            ...
            if imgOriginal is not None:
                ...
                if angle < 5:
                    ...
                    resizedShape = (250, (int)(originalShape[1] * 250.0 / originalShape[0]))
                    imageResized = cv.resize(imgOriginal, (resizedShape[1], resizedShape[0]))
                    imageProcessed = PreprocessImage.preprocess(imageResized, False)
                    path = os.path.join(savePath, str(i), str(j))
                    
                    coeffX = 250.0 / originalShape[0]
                    coeffY = resizedShape[1] / originalShape[1]
                    cropboxResized = (np.array([x1 * coeffX, y1 * coeffY, x2 * coeffX, y2 * coeffY])).astype(int)
                    imageCropped[i].append(PreprocessImage.preprocess(cv.resize(imgCropped, (224, 224)), False))
                    
                    np.savez_compressed(path, a = imageProcessed)
                    cropbox[i].append(cropboxResized)
